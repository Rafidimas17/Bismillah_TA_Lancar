...
try:
    log = open('dataReport.txt',"w")
except:
    print( "Data Gagal Disimpang")
...
print( "Enter line y:",str(line_down))
print( "Exit line y:", str(line_up))
...
while(cap.isOpened()):
                ...
                for i in persons:
                    if abs(x-i.getX()) <= w and abs(y-i.getY()) <= h:
                        ...
                        if i.going_UP(line_down,line_up) == True:
                            cnt_up += 1;
                            print( "ID:",i.getId(),'crossed going exit at',time.strftime("%c"))
                            log.write("ID: "+str(i.getId())+' crossed going exit at ' + time.strftime("%c") + '\n')
                        elif i.going_DOWN(line_down,line_up) == True:
                            cnt_down += 1;
                            print( "ID:",i.getId(),'crossed going entered at',time.strftime("%c"))
                            log.write("ID: " + str(i.getId()) + ' crossed going entered at ' + time.strftime("%c") + '\n')
                        break
                    if i.getState() == '1':
                        if i.getDir() == 'entered' and i.getY() > down_limit:
                            i.setDone()
                        elif i.getDir() == 'exit' and i.getY() < up_limit:
                            i.setDone()
                    if i.timedOut():
    ...
    cv.imshow('Counting People',frame)
    # cv.imshow('Mask',mask)
